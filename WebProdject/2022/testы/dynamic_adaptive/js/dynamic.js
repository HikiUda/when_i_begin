//Dynamic Adapt v.1
//HTML	ddta-da="where(uniq class name),position(diget),when(breakpoint)"
//e.x. data-da="item,2,992"
//Andrikanych Yevhen 2020
//htpp://www.youtube.com/c/freelancerlifestyle
	...
	if (da_elements.length > 0) {
		...
		for (let index = 0; index < da_elements.length; index++) {
			...
			if (da_array.length == 3) {
				...
				//Заполняем массив пераоначальных позиций
		...
		for (let index = 0; index < da_elements_array.length; index++) {
			...
			da_match_media.puch(window.matchMedia("(" + da_type + "-width: " + da_breakpoint + "px"));
...
function custom_adapt() {
	const viewport_windth = Math.max(document.documentElement.clientWidth, window.innerWidth || 0);
}
