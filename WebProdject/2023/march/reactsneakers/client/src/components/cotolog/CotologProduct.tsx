...
interface ProductCard {
   product: IProducts | IPurchesProducts;
   disabled: boolean;
}

const CotologProduct: FC<ProductCard> = ({ product, disabled }) => {
   const dispatch = useAppDispatch();

   const { title, price, img, inCart, isLiked } = product;
   const [liked, setLiked] = useState(false);
   const [cart, setCart] = useState(false);

   async function changeLiked() {
      setLiked(!liked);
      await putSneakers(cart, !liked, product);
   ...
   return (
      <div className="cotolog__product-item product-item">
         <button disabled={disabled} onClick={changeLiked} className="product-item__liked">
            {liked ? (
               <svg
                  width="32"
                  height="32"
         ...
         <div className="product-item__product-state">
            <div className="product-item__cost">
               <span>Цена:</span>
               <div className="product-item__price">{price}</div>
            </div>
            <button disabled={disabled} onClick={changeCart} className="product-item__purchased">
               {cart ? (
                  <svg
                     width="32"
                     height="32"
